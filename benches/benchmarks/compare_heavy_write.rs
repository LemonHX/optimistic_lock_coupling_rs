...
fn heavy_write_list_mutex(i: i32) {
    ...
    let _ = thread4.join();
    let _ = thread5.join();
    unsafe { assert_eq!(lock.as_ref().unwrap().get2(), 3 * i) }
}
fn heavy_write_list_optimistic_lock_coupling(i: i32) {
...
fn lock_heavy_write_list(c: &mut Criterion) {
    let mx = Fun::new("Mutex", |b, i| b.iter(|| heavy_write_list_mutex(*i)));
    let ol = Fun::new("OptimisticLockCoupling", |b, i| {
        b.iter(|| heavy_write_list_optimistic_lock_coupling(*i))
    });

    let functions = vec![ol, mx];

    c.bench_functions("Lock Heavy Write List Compare", functions, 1_000_000);
}

criterion_group!(name = lock_heavy_write; config = Criterion::default().sample_size(1_000); targets = lock_heavy_write_list);
